<?php 
  ...
  function predict_for_opening_price($stock){
	...
	$stock_upper= strtoupper($stock);

    for ($i=0; $i < 4 ; $i++) { ?> <table style="border:1px solid black"><tr><th> <?php
      echo "<p>There is a " . ($probabilities[$i]*100). "% chance that the tomorrow's opening price of ". $stock_upper." will lie from $" . $probable_range[$i] . " to $" . $probable_range[$i+1] . "</p>";
      ?> </th></tr></table><?php
	  //echo "<br/>";
    }
  }
  ...
  function recommendation_for_close_price($stock){
    $bin = find_bin($stock,"close");
	...
	$close_rec=($probable_range[3]+$probable_range[4])/2;

	return $close_rec;
  }
  
  function recommendation_for_open_price($stock){
    $bin = find_bin($stock,"open");
	...
	$open_rec=($probable_range[3]+$probable_range[4])/2;

	return $open_rec;
  }
  
  function recommendation($stock){ 
	  $open_rec=recommendation_for_open_price($stock);
	  $close_rec=recommendation_for_close_price($stock);
	  
	  $rec=$close_rec-$open_rec;
...
function test($stock){
	...
	return array($bin,$freq,$cum_freq,$range,$p_states,$transition_states,$state_count,$transition_matrix,$probabilities,$probable_range);
}
?>